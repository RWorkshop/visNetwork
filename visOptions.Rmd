...
highlightNearest
: Custom Option. Just a Boolean, or a named list. Default to false. Highlight
...
algorithm, you can also pass a list(from = 1, to = 1) to control
degree in both direction
...
all nodes, without taking direction information. "hierarchical" look only at
...
Default to TRUE
nodesIdSelection
: Custom Option. Just a Boolean, or a named list. Default to false. Add an id
node selection creating an HTML select element. This options use click event.
Not available for DOT and Gephi.
-  "enabled" : Boolean. Default to false. Activated or not ?.
-  "values : Optional. Vector of possible values (node’s id), and so order is
preserve. Defaut to all id in nodes data.frame.
-  "selected" : Optional. Integer/Character. Initial id selection. Defaut to
NULL
-  "style" : Optional. Character. HTML style of list. Default to ’width: 150px;
height: 26px’.
-  "useLabels" : Optional. Boolean. Use labels instead of id ? Default to
TRUE.
-  "main" : Optional. Default to "Select by id"
selectedBy : Custom option. Character or a named list. Add a multiple selection based on
column of node data.frame creating an HTML select element. Not available for
DOT and Gephi.
70 visOptions
